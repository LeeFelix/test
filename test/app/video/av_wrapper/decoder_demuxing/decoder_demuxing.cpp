...
int DecoderDemuxer::init(char* src_filepath, uint32_t flag) {
  ...
  if (open_codec_context(&video_stream_idx, fmt_ctx, AVMEDIA_TYPE_VIDEO) >= 0) {
    ...
    assert(pix_fmt == AV_PIX_FMT_NV12);  // rk_vpu always return NV12 data
    assert((width & 15) == 0 && (height & 1) == 0);  // must align
  }
...
#if 0
            {
                FILE *audio_dst_file = fopen("/mnt/sdcard/audio.pcm", "ab");
                assert(audio_dst_file);
...
int DecoderDemuxer::rewind() {
  int ret = avformat_seek_file(fmt_ctx, -1, INT64_MIN, 0, INT64_MAX,
                               AVSEEK_FLAG_BYTE);
  if (0 == ret) {
    finished = false;
  }
...
extern "C" int decode_one_video_frame(void* ctx,
  ...
  if (frame) {
    ...
    while (!got_frame) {
      ret =
          dd->read_and_decode_one_frame(image_buf, frame, &got_frame, &is_video);
      if (dd->is_finished()) {
        ret = -1;
        break;
      }
