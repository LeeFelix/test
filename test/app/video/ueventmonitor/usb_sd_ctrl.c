...
void android_usb_init(void)
{
	...
	int pid = -1;
	char *argv[] = {"adbd",  NULL};
	char *envp[] = {"PATH=/bin:/usr/local/sbin", NULL};
	...
	if (pid == 0) {
		umask(077);
		setpgid(0, getpid());
		if (execve("/usr/local/sbin/adbd", argv, envp) < 0)
			printf("cannot execve('%s'): %s\n", argv[0],
			       strerror(errno));

		printf("Error : never reach here.\n");
	}

	if (pid < 0)
		printf("Failed to start adbd\n");
}
