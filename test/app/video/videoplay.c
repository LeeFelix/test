...
static void display(int rga_fd,
  ...
  if (outdevice == OUT_DEVICE_HDMI || outdevice == OUT_DEVICE_CVBS)
    ret = rk_rga_ionfdnv12_to_ionfdnv12_rotate(rga_fd, fd, w, h, stride_w,
                                               stride_h, win->video_ion.fd,
                                               out_w, out_h, 0);
  else
    ret = rk_rga_ionfdnv12_to_ionfdnv12_rotate(
        rga_fd, fd, w, h, stride_w, stride_h, win->video_ion.fd,
        win->video_ion.width, win->video_ion.height,
...
static void video_image_display(VideoState* is) {
  ...
  if (vp->vpu_mem_handle) {
    ...
    if (handle) {
      if (handle->image_buf.phy_addr > 0) {
        AVCodecContext* v_codec_ctx = is->viddec.avctx;
        display(is->rga_fd, handle->image_buf.phy_addr, vp->width, vp->height,
...
static int video_open(VideoState* is, int force_set_video_mode, Frame* vp) {
  if (show_status)
    fprintf(stderr, "---video_open TODO \n");
  return 0;
}
...
static void update_volume(VideoState* is, int sign, int step) {
  is->audio_volume = av_clip(is->audio_volume + sign * step, 0, MIX_MAXVOLUME);
}
        ...
        return ret;
            ...
            if(is->last_display_time > 0.0) {
                if (delay < time-is->last_display_time) {
                    ...
                    *remaining_time = FFMIN(delay-(time-is->last_display_time), *remaining_time);
                }
            }
#endif
      ...
      if (frame->pts != AV_NOPTS_VALUE) {
        ...
        if (!isnan(diff) && fabs(diff) < AV_NOSYNC_THRESHOLD &&
            ...
            ((get_master_sync_type(is) != AV_SYNC_VIDEO_MASTER &&
              diff + time_base * 1000 < 0) ||
             (speed != 1 && diff <= time_base * (speed - 1) * 1000)) &&
#endif
            is->viddec.pkt_serial == is->vidclk.serial &&
  ...
  if (is->swr_ctx) {
    ...
    if (wanted_nb_samples != af->frame->nb_samples) {
      if (swr_set_compensation(
              is->swr_ctx, (wanted_nb_samples - af->frame->nb_samples) *
...
#if RK_DEBUG_CLOCK
    printf("before set audio_clock in callback\n");
#endif
  ...
  while (is->audio_out_running) {
    if (!is->paused) {
...
#endif
      if (audio_params_changed(is->waparams.fmt,
                               is->waparams.wanted_channel_layout,
                               is->waparams.wanted_sample_rate)) {
        AudioParams* hw_params = NULL;
        if (audio_dev_open(is->waparams.fmt, is->waparams.wanted_channel_layout,
                           is->waparams.wanted_sample_rate, &hw_params)) {
          goto err;
        }
        is->audio_hw_buf_size = hw_params->audio_hw_buf_size;
        is->audio_src = *hw_params;
        is->audio_tgt = *hw_params;
        if (len != is->audio_hw_buf_size) {
          len = is->audio_hw_buf_size;
          if (stream_buf)
            av_free(stream_buf);
          stream_buf = (uint8_t*)av_malloc(len);
          if (!stream_buf)
            goto err;
          frames_left = is->audio_tgt.pcm_samples;
        }
      ...
      while (frames_left > 0) {
        ...
        stream += status;
#if RK_DEBUG_FRAME_QUEUE
        printf("----frames_left: %d, duration: %f\n", frames_left,
               av_gettime_relative() / 1000000.0 - curtime);
#endif
      }
    } else {
      av_usleep(1000);
...
static int audio_open(void* opaque,
  ...
  return audio_dev_open(fmt, wanted_channel_layout, wanted_sample_rate,
                        audio_hw_params);
}
...
static int stream_component_open(VideoState* is, int stream_index) {
  ...
  struct AudioParams* hw_params = NULL;
  ...
  switch (avctx->codec_type) {
...
#if CONFIG_AVFILTER
    {
      ...
      fmt = avctx->sample_fmt;
#endif

      /* prepare audio output */
      if ((ret = audio_open(is, fmt, channel_layout, nb_channels, sample_rate,
                            &hw_params)) < 0) {
        // av_assert0(0);
      ...
      is->waparams.fmt = fmt;
      is->waparams.wanted_channel_layout = channel_layout;
      is->waparams.wanted_sample_rate = sample_rate;
...
static void seek_chapter(VideoState* is, int incr) {
  ...
  av_log(NULL, AV_LOG_ERROR, "Seeking to chapter %d.\n", i);
...
void videoplay_set_fb_black() {
  ...
  for (i = 0; i < 2; i++) {
    win = rk_fb_getvideowin();
#if 0
    if (win->width != win->video_ion.width ||
        win->height != win->video_ion.height) {
      video_ion_free(&win->video_ion);
      printf("alloc ion buffer: %dx%d\n", win->width, win->height);
      if (video_ion_alloc(&win->video_ion, win->width, win->height)) {
        printf("%s:ion alloc fail\n", __func__);
      }
    }
#endif
    video_ion_buffer_black(&win->video_ion);
    if (rk_fb_video_disp(win) == -1) {
      printf("rk_fb_video_disp err\n");
    }
  }
...
static void draw_black_to_screen() {
  ...
  if (rk_fb_video_disp_ex(video_win, video_win->video_ion.width,
                          video_win->video_ion.height))
...
int videoplay_decode_one_frame(unsigned char* filepath) {
  ...
  DataBuffer_VPU_t vpu_data_buf = {0};
  void* ctx = NULL;
  int rga_fd = -1;
  if (!filepath) {
    return -1;
  ...
  if (!ret) {
    // init_video_win2(w, h);
    display(rga_fd, vpu_data_buf.image_buf.phy_addr, w, h, coded_w, coded_h);
...
void videoplay_decode_jpeg(unsigned char* filepath) {
  ...
  unsigned int fb_width, fb_height, temp;
  int out_w, out_h;
  int outdevice;
...
#if 0
  if (fb_width != video_win->video_ion.width ||
      fb_height != video_win->video_ion.height) {
    video_ion_free(&video_win->video_ion);
    if (video_ion_alloc(&video_win->video_ion, fb_width, fb_height)) {
      printf("%s video_ion_alloc fail!\n", __func__);
      goto exit1;
    }
  }
  ...
  outdevice = rk_fb_get_out_device(&out_w, &out_h);
  if (outdevice == OUT_DEVICE_HDMI || outdevice == OUT_DEVICE_CVBS)
    rk_rga_ionfdnv12_to_ionfdnv12_rotate_ext(
        rotate.fd, width, height, (width + 15 & ~15), (height + 15) & ~15,
        video_win->video_ion.fd, out_w, out_h, 0);
  else
    rk_rga_ionfdnv12_to_ionfdnv12_rotate_ext(
        rotate.fd, width, height, (width + 15 & ~15), (height + 15) & ~15,
        video_win->video_ion.fd, video_win->video_ion.width,
        video_win->video_ion.height, VIDEO_DISPLAY_ROTATE_ANGLE);
...
exit1:
  ...
  if (fb_width != video_win->video_ion.width ||
      fb_height != video_win->video_ion.height) {
    video_ion_free(&video_win->video_ion);
    if (video_ion_alloc(&video_win->video_ion, width, height)) {
      printf("%s video_ion_alloc fail!\n", __func__);
      goto exit;
    }
  }
